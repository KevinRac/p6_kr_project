...
if ( sudo useradd -p $(openssl  passwd -crypt projet6) -m Utilisateur)
...
if (sudo useradd -p $(openssl  passwd -crypt projet6) -m P6_Admin )
	then echo -e " --> L'utilisateur Admin et ses droits ont été créés."
else 
	echo -e " --> Erreur lors de la création de l'utilisateur Admin ou il est déjà créé ."
fi
